...
module.exports = (client) => {
	try {
										...
										for (const option of pull.options) {
											if (option.User && option.User.name && option.User.description) {
												...
												subcommand.addStringOption((op) =>
													op.setName(String(option.String.name).replace(/\s+/g, '_').toLowerCase()).setDescription(option.String.description).setRequired(option.String.required).setAutocomplete(option.String.autocomplete || false)
												)
		...
		client.on("ready", () => {
			// load 1 lần thôi load nhiều dính rate limit 200 lệnh lại cười :)
